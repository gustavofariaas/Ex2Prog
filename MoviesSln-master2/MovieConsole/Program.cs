    ...
    class Program
    {
        ...
        static void Main(String[] args)
        {
           ...
           // var q2 = db.Movies.Where(f => f.Director == "Quentin Tarantino").Select(f => f.Title);
            ...
            Console.WriteLine("Nomes dos atores do filme Jurassic Park:");
            var parque = from f in db.ActorMovies
                         where f.Movies.Where(x => x.Title == "Jurassic Park").Select(x => x.Title).FirstOrDefault() == "Jurassic Park"
                         select new
                     {
                         Atores = f.Actors
                     };
            foreach (var elenco in parque){
                foreach(var atores in elenco.Atores)
                {
                    Console.WriteLine(atores.Name);
                }
            }

            Console.WriteLine("Nomes dos atores que ja foram agentes 007:");
            var agentes = from f in db.ActorMovies
            ...
            foreach (var atores in agentes)
                {
                var at = from f in db.Actors where f.ActorId == atores.Ator select f.Name;
                Console.WriteLine(at);
                 
                }

            Console.WriteLine("Ator que mais foi agente 007");
            var ag = from f in db.ActorMovies
                     where f.Character == "James Bond"
                     select f.ActorId;
       ...
       Console.WriteLine("Filmes de Eva Green");
            var film = from f in db.ActorMovies
                       where f.Actors.Where(x => x.Name == "Eva Green").Count() > 0
                       select new
                       {
                           Nome = f.MovieId
                       };
            foreach (var elenco in film)
            {
                var at = from f in db.Movies where f.MovieId == elenco.Nome select f.Title;
                foreach (var nm in at)
                    Console.WriteLine(nm);
            }

            Console.WriteLine("Generos de Filmes de Eva Green");
